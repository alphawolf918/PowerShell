...
function CompressJpegFolder{
    ...
    foreach($file in $allChild)
    {
        ...
        $outFile = Join-Path -Path $outfolderPath -ChildPath ($file.BaseName + ".jpg")

        Save-Jpeg $file.FullName $outFile $quality
		
		convert -scale 525 $file.FullName $outFile
    }

}

CompressJpegFolder "C:\Users\pshannon\Documents\PowerShell" "C:\Users\pshannon\Documents\PowerShell\Output" 35